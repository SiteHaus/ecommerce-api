...
import { handleOrderShipped } from "./order-shipped.handler";
...
jest.mock("@sitehaus-ecom/email-templates", () => ({
  OrderShipped: jest.fn().mockReturnValue("order-shipped-element"),
}));
...
describe("handleOrderShipped", () => {
  ...
  it("renders the street fetched from Stripe, not the (now empty) column", async () => {
    ...
    await handleOrderShipped(job, ctx);

    const props = (OrderShipped as jest.Mock).mock.calls[0][0];
