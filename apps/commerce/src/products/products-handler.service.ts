...
export class ProductsHandlerService {
  ...
  async listPublic(data: PublicQueryParams & { storeId: string }) {
    ...
    const [allImages, allVariants] = await Promise.all([
      ...
      this.db.query.productVariantsTable.findMany({
        where: (v, { inArray }) => inArray(v.productId, productIds),
        with: { inventory: true },
      }),
    ]);
    ...
    const items = products.map((p) => {
      ...
      const primaryImage =
        allImages.find((img) => img.productId === p.id) ?? null;
      ...
      return {
        ...
        variants: scheduled
          ...
          : variants.map((v) => ({
              ...
              availability: getAvailability(v.inventory),
            })),
      };
    });
  ...
  async getProduct(data: { id: string; storeId: string }) {
    ...
    const product = await this.db.query.productsTable.findFirst({ where });
